...
if __name__ == "__main__":
    # cgp = CgK(k=3, plotbasis=True)
    cgp = CgK(k=1)
    # test_wave(method=cgp)
    import applications, applications_analytical_solution
    import inspect

    ac = {}
    ac['applications_analytical_solution'] = [cls_name for cls_name, cls_obj in inspect.getmembers(sys.modules['applications_analytical_solution']) if
               inspect.isclass(cls_obj)]
    ac['applications'] = [cls_name for cls_name, cls_obj in inspect.getmembers(sys.modules['applications']) if
               inspect.isclass(cls_obj)]

    ac = {'applications_analytical_solution': ['Oscillator']}
    for k, classes in ac.items():
        for classe in classes:
            app =  eval(f"{k}.{classe}()")
            t = np.linspace(app.domain[0], app.domain[1], 100)
            u_node, u_coef = cgp.run_forward(t, app)
            if hasattr(app, 'plot'):
                app.plot(t, u_node)
            app.plot_solution(t=t, u=u_node, show=True)

    # app = applications.Lorenz(t_end=30)
    # t = np.linspace(app.domain[0], app.domain[1], 400)
    #
    # app = applications_analytical_solution.Oscillator()
    # t = np.linspace(app.domain[0], app.domain[1], 40)
    #
    # app = applications.Pendulum(t_end=20, is_linear=False)
    # t = np.linspace(app.domain[0], app.domain[1], 100)
    #
    # u_node, u_coef = cgp.run_forward(t, app)
    # if hasattr(app, 'plot'):
